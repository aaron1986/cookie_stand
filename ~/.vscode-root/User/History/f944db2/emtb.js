...
form.addEventListener("submit", function (event) {
    ...
    const minCookies = parseInt(event.target.minCookies.value);
    const maxCookies = parseInt(event.target.maxCookies.value);
    const avgCookies = parseInt(event.target.avgCookies.value);


    //console.log(location, minCookies, maxCookies, avgCookies);
    const newStore = new ShopLocation(location, minCookies, maxCookies, avgCookies);

    newStore.calcTotal();
    shopLocations.push(newStore);
    createShopLocation(newStore); 



/*     const numArray = [];

    const numberInput = document.getElementById("min-cookies");
    const inputvalue = parseInt(numberInput.value);
    console.log(inputvalue); */

    //let result = (minCookies + maxCookies) / 2;
    //console.log((minCookies + maxCookies) / 2);
